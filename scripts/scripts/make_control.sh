...
do
    ...
    do
        if [ $bbdd == 0 ]
        then
            echo "res/networks/${net}.xbif res/networks/BBDD/${net}.xbif_.csv" >> control_experiments.txt
        else
            echo "res/networks/${net}.xbif res/networks/BBDD/${net}.xbif${bbdd}_.csv" >> control_experiments.txt
        fi
        #echo ""
